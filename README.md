...
- 배팅 금액 조건
    - [ ] 블랙잭(2장 합 21)일 경우 배팅 금액을 돌려받고 배팅 금액의 `1.5배`를 추가로 받는다.
    - [ ] 플레이어가 승리하면 배팅 금액 만큼 받는다.
    - [ ] 플레이어가 무승부이면 금액을 받지 않는다.
    - [ ] 플레이어가 패배할 경우 배팅 금액을 모두 잃게 된다.
    - [ ] 딜러는 승리한 플레이어에게 금액을 지불하고 패배한 플레이어의 배팅 금액을 수금한다.
    - [ ] 딜러의 최종 수익은 `패배한 플레이어의 배팅 금액 - 승리한 플레이어에게 지불한 금액` 이다.
