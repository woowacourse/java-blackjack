...
- [x] getter의 연쇄 호출 수정
- [x] 단순히 생성자를 호출할 뿐인데, 이런 방식의 구조와 이름을 사용하신 이유
...
- [ ] 불변성이 왜 좋은지, 불변성을 지킬 수 있다면 무조건 좋은지 고민
    - [ ] 그리고 현실적으로 게임 진행을 위해 컨트롤러에서 개별 플레이어를 알아야 하는 상황
    - [ ] 현재 플레이어는 participant가 가지고 있기 때문에 이렇게 플레이어를 달라고 요청
    - [ ] participant 역할은 무엇인지
