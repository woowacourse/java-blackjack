...
- [x] 게임에 참여할 플레이어의 이름을 입력받고, 쉼표를 기준으로 구분한다.
    ...
    - [x] 플레이어의 이름이 null이거나 공백이라면 예외를 발생시키고 재입력받는다.
...
- [x] 딜러와 플레이어별로 최종 수익을 출력한다.
    - [x] 플레이어가 블랙잭이고 딜러가 블랙잭이 아니라면 플레이어는 배팅금액의 1.5배를 받고 딜러는 플레이어의 해당 금액만큼 잃는다.
    - [x] 플레이어가 이겼다면 배팅금액만큼 받고, 딜러는 플레이어의 배팅금액만큼 잃는다.
    - [x] 플레이어가 졌다면 배팅금액을 잃고, 딜러는 플레이어의 배팅금액만큼 받는다.
    - [x] 비겼다면 플레이어는 배팅금액을 돌려받는다.
