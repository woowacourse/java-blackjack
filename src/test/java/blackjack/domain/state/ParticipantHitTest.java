...
public class ParticipantHitTest {

    @Test
    @DisplayName("카드 두 장을 받고 블랙잭이 아니면 hit가 된다.")
    void hit() {
        State state = Ready.dealToParticipant(SPADE_ACE, SPADE_TWO);
    ...
    void hit_bust() {
        State state = Ready.dealToParticipant(SPADE_THREE, SPADE_NINE)
                .draw(SPADE_TEN);
    ...
    void hit_not_blackjack_21() {
        State state = Ready.dealToParticipant(SPADE_NINE, SPADE_TWO)
                .draw(SPADE_TEN);
    ...
    void hit_stay() {
        State state = Ready
                .dealToParticipant(SPADE_ACE, SPADE_TWO)
                .stay();
