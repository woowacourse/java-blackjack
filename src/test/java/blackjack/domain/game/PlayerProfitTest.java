...
class PlayerProfitTest {
    ...
    void canCreateWhenPlayerBlackjackWithInitialCard() {
        ...
        assertThat(profit.getProfit()).isEqualTo((int) (player.getBettingAmount() * 1.5d));
    }
    ...
    void canCreatePlayerBust() {
        ...
        assertThat(profit.getNickname()).isEqualTo("플레이이어");
        assertThat(profit.getProfit()).isEqualTo(player.getBettingAmount() * -1);
    }
    ...
    void canCreateWhenDealerBust() {
        ...
        assertThat(profit.getNickname()).isEqualTo("플레이이어");
        assertThat(profit.getProfit()).isEqualTo(player.getBettingAmount());
    }

    @ParameterizedTest
    @DisplayName("승패에 따라 이익을 구할 수 있다.")
    @MethodSource()
    void canCreateByWinningType(WinningType winningType, int multiple) {
        Player player = new Player(new Nickname("플레이이어"));
        player.registerBettingAmount(new BettingAmount(1000));
        ...
        assertThat(profit.getNickname()).isEqualTo("플레이이어");
        assertThat(profit.getProfit()).isEqualTo(player.getBettingAmount() * multiple);
    }

    static Stream<Arguments> canCreateByWinningType() {
        return Stream.of(
                Arguments.of(WinningType.WIN, 1),
                Arguments.of(WinningType.LOSE, -1),
                Arguments.of(WinningType.DRAW, 0)
        );
    }
}