...
class BlackJackGameTest {
    ...
    void drawDefaultCardTest() {
        ...
        assertThat(dealer.getCards()).isEqualTo(List.of(SpadeCard.SPADE_FOUR, DiamondCard.DIAMOND_SEVEN));
    }
    ...
    void drawDealerCardUntilSatisfyingConditionTest() {
        ...
        List<Card> expectedCards = List.of(CloverCard.CLOVER_FIVE, HeartCard.HEART_JACK, SpadeCard.SPADE_FOUR);

        assertThat(dealer.getCards()).isEqualTo(expectedCards);
    }

    @DisplayName("플레이어가 버스트일 경우, 딜러가 승리한다.")
    @Test
    void judgeWinnerTest_playerBust() {
        Map<Boolean, Integer> winningRecord = blackJackGame.getDealer().getWinningRecord();
        Player player = blackJackGame.getPlayers().get(0);
        blackJackGame.drawDefaultCard(); // 플레이어 : 5클로버, J하트 , 딜러 : 4스페이드, 7다이아몬드
        blackJackGame.drawOneMoreCard(player); // 플레이어 : 5클로버, J하트, A하트
        blackJackGame.drawOneMoreCard(player); // 플레이어 : 5클로버, J하트, A하트, 6클로버

        blackJackGame.judgeWinner();

        assertThat(player.isWinner()).isFalse();
        assertThat(winningRecord.get(true)).isEqualTo(1);
        assertThat(winningRecord.get(false)).isEqualTo(0);
    }

    @DisplayName("플레이어는 버스트가 아니고, 딜러는 버스트일 경우 플레이어가 승리한다.")
    @Test
    void judgeWinnerTest_playerNormalAndDealerBust() {
        ...
        Dealer dealer = blackJackGame.getDealer();
        blackJackGame.drawDefaultCard(); // 플레이어 : 5클로버, J하트 , 딜러 : 4스페이드, 7다이아몬드
        dealer.receiveCards(List.of(HeartCard.HEART_TEN, DiamondCard.DIAMOND_TEN)); // 딜러 : 4스페이드, 7다이아몬드, 10하트, 10다이아몬드


        blackJackGame.judgeWinner();

        assertThat(player.isWinner()).isTrue();
        assertThat(winningRecord.get(true)).isEqualTo(0);
        assertThat(winningRecord.get(false)).isEqualTo(1);
    }

    @DisplayName("딜러의 점수가 플레이어의 점수보다 높을 경우, 딜러가 승리한다.")
    @Test
    void judgeWinnerTest_dealerWin() {
        Map<Boolean, Integer> winningRecord = blackJackGame.getDealer().getWinningRecord();
        Player player = blackJackGame.getPlayers().get(0);
        blackJackGame.drawDefaultCard(); // 플레이어 : 5클로버, J하트 , 딜러 : 4스페이드, 7다이아몬드
        blackJackGame.drawDealerCardUntilSatisfyingCondition(); // 딜러 : 4스페이드, 7다이아몬드, A하트, 6클로버
    ...
    @DisplayName("플레이어의 점수가 딜러의 점수보다 높을 경우, 플레이어가 승리한다.")
    @Test
