...
### 게임 결과, 심판
게임이 종료되었을 때 나올 수 있는 상태는 버스트, 스탠드, 블랙잭 세 종류이다. 각각의 상황에 따른 상태를 정의한다.
각각의 상태를 클래스로 만들어 심판이 올바른 상태를 확인한다.
- [x] 게임 결과를 아래의 순서대로 올바르게 판단한다.
- [x] 플레이어가 **블랙잭**인 경우, **딜러가 블랙잭**이면 무승부이고, 그렇지 않으면 블랙잭 승리한다.
- [x] 플레이어가 **버스트**인 경우, 항상 진다.
- [x] 플레이어가 **스탠드**인 경우, **딜러가 버스트**이면 승리하고, 그렇지 않으면 **딜러와 점수를 비교**하여 승패를 판단한다.
