...
public class InputView {
    ...
    private static final String MESSAGE_ASK_BETTING_AMOUNT = "의 베팅 금액은?";
    ...
    private static final String ERROR_MESSAGE_NOT_NUMBER_FORMAT = "[ERROR] 배팅 금액은 숫자여야 합니다.";
    private static final String ERROR_MESSAGE_NOT_NEGATIVE = "[ERROR] 배팅 금액은 음수일 수 없습니다.";
    ...
    public int askBettingAmount(String playerName) {
        System.out.println(playerName + MESSAGE_ASK_BETTING_AMOUNT);
        try {
            String input = scanner.nextLine();
    ...
    private void checkIsNumberFormat(String input) {
        try {
            ...
            throw new IllegalArgumentException(ERROR_MESSAGE_NOT_NUMBER_FORMAT);
        }
    }

    private void checkNotNegative(String input) {
        if (Integer.parseInt(input) < 0) {
            throw new IllegalArgumentException(ERROR_MESSAGE_NOT_NEGATIVE);
        }
    }
