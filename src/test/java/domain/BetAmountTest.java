...
public class BetAmountTest {
    ...
    void createPlayerBetInvalidInput(String value) {
        assertThatThrownBy(() -> new BetAmount(value)).isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void createPlayerBetValidInput(String value) {
        assertThatCode(() -> new BetAmount(value)).doesNotThrowAnyException();
    }

    @DisplayName("10000원 미만은 베팅할 수 없다.")
    @Test
    void createPlayerBetByMinValue() {
        assertThatThrownBy(() -> new BetAmount("0")).isInstanceOf(IllegalArgumentException.class);
    }
}
