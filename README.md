...
- [ ] `Cards`에서 다른 `Cards`와 비교를 통해 승/무/패를 관리 - 카일

- 블랙잭 게임이 시작되면 항상 모든 플레이어는 2장의 카드를 가지게 됩니다. READY 라는 상태를 두어서 가지고 있는 Cards 가 불완전한 상태를 만드는 것이 득이 되는게 있을까요?
- 플레이어가 버스트 되면 딜러가 무조건 이긴다는 규칙이 반영되지 않은 것 같아요.
