...
public class OutputView {
    ...
    private static final String DRAW_TWO_CARDS_FORM = "%s와 %s에게 2장을 나누었습니다." + System.lineSeparator();
    private static final String CARD_PRINT_FORM = "%s : %s" + System.lineSeparator();
    private static final String FINAL_CARD_PRINT_FORM = "%s : %s - 결과 : %d" + System.lineSeparator();
    ...
    public static void drawTwoCards(ShowDeckDto dealer, List<ShowDeckDto> participants) {
        String names = participants.stream().map(ShowDeckDto::name).collect(Collectors.joining(JOIN_DELIMITER));
