...
- [x] 플레이어의 결과를 토대로 딜러의 승무패 결과를 반환한다.
...
7. 상속이 가지는 많은 문제점들 때문에 조합으로 구현해보았습니다. 하지만, 막상 구현하고 보니 요구사항 중에 `딜러와 플레이어에서 발생하는 중복 코드를 제거해야 한다.`를 지키지 못하는 것 같았어요. 왜냐하면 같은 로직의 메서드가 존재하기 떄문이에요. 그리고 코드 재사용을 목적으로 상속을 사용하게 되면 부모와 자식 클래스 간의 강한 결합도가 생겨서 변경 및 확장성이 매우 안좋아지기에 조합을 쓴다고 공부했는데요. 현재 코드에서 조합으로 구현해도 이 부분에 대한 이점을 느끼지 못했습니다. 어떤 부분에서는 상속으로 했으면 더 편하고 깔끔했겠다는 느낌도 받게 되는데요. 이에 관련하여 페어와 많은 이야기를 나눠봤지만, 페어도 어떤 점이 좋아서 조합을 쓰라고 하는 지 모르겠다고 합니다. 혹시 알려주실 수 있으실까요 😢
