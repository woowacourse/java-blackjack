...
public class OutputView {
	...
	public static void printResult(Result result, Dealer dealer, Players players) {
		...
		players.forEach(player -> System.out.println(player + ": " + result.get(player).getName()));
	}

	private static void printResultStatus(Cards dealerCards, Players players) {
		System.out.println("딜러: " + dealerCards.toList().stream()
			.map(Card::toString)
			.collect(joining(DELIMITER)) +
			" - 결과: " + Score.of(dealerCards));
