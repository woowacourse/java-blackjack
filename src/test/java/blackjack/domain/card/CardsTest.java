...
public class CardsTest {
	...
	@Test
	@DisplayName("카드가 2장 미만이면 시작상태가 아니다.")
	void is_not_started() {
		List<Card> list = new ArrayList<>();
		list.add(Card.valueOf(Denomination.ACE, Suit.CLOVER));
		Cards cards = new Cards(list);

		assertThat(cards.isReady()).isFalse();
	}

	@Test
	@DisplayName("카드가 2장 이상이면 시작상태이다.")
	void is_started() {
		List<Card> list = new ArrayList<>();
		list.add(Card.valueOf(Denomination.ACE, Suit.CLOVER));
		list.add(Card.valueOf(Denomination.NINE, Suit.CLOVER));
		Cards cards = new Cards(list);

		assertThat(cards.isReady()).isTrue();
	}
