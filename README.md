...
- [x] 배팅 결과를 계산해서 반환한다.
  - [x] 딜러가 블랙잭인 경우
    - [x] 버스트된 플레이어는 배팅 금액만큼 잃는다.
    - [x] 버스트되지 않고 블랙잭이 아닌 플레이어는 배팅 금액만큼 잃는다.
    - [x] 블랙잭인 플레이어는 배팅 금액의 1.5배를 추가로 받는다.
  - [x] 딜러가 버스트인 경우
    - [x] 버스트된 플레이어는 배팅 금액을 잃지도 얻지도 않는다.
    - [x] 버스트되지 않고 블랙잭이 아닌 플레이어는 배팅 금액의 1배를 추가로 받는다.
    - [x] 블랙잭인 플레이어는 배팅 금액의 1.5배를 추가로 받는다.
  - [x] 딜러가 버스트도 블랙잭도 아닌 경우
    - [x] 점수가 딜러보다 높은 플레이어는 배팅 금액의 1배를 추가로 받는다.
    - [x] 점수가 딜러보다 낮은 플레이어는 배팅 금액만큼 잃는다.
    - [x] 점수가 딜러와 같은 플레이어는 배팅 금액을 잃지도 얻지도 않는다.
