...
3. 플레이어 액션: 플레이어들이 차례대로 액션을 진행합니다.
    - 플레이어는 점수를 맞추기 위해 카드를 더 받거나 그만 받을 수 있습니다.
4. 딜러 액션: 모든 플레이어의 액션이 끝나면 딜러의 액션을 진행합니다.
    - 딜러는 점수가 17점 미만인 경우 무조건 카드 1장을 추가로 받으며, 그 이상인 경우 카드를 그만 받습니다.
5. 결과: 딜러의 점수와 플레이어들의 점수를 각각 비교하여 높은 쪽이 승리합니다.
...
- 액션 이후: 카드 두 장 이상일 때
    ...
    - 딜러만 버스트 -> 승리 ✅
    - 플레이어랑 딜러 둘다 버스트 -> 패배 ✅
    - 플레이어랑 딜러 둘다 버스트 아님 -> 점수가 21점에 더 가까우면 승리, 점수가 같으면 푸시 ✅

### 카드 딜링 이후: 카드 두 장일 때

초반에 모두가 2장씩 나눠가졌을 때 발생할 수 있는 경우의 수는 아래와 같습니다.

참고로 초반에 두 장씩만 나눠 가졌을 땐 파산(점수가 22 이상)이 나올 수 없어 파산을 고려하지 않아도 됩니다.
...
1. [테이블 참가] 입력 받은 이름의 플레이어들을 등록한다 ✅
    ...
    - 플레이어 이름은 한 글자 이상이다 ✅
    - 플레이어 이름은 중복될 수 없다 ✅
2. [베팅] 각 플레이어들의 베팅 금액을 등록한다 ✅
    - 플레이어 이름으로 플레이어의 베팅 금액을 등록한다 ✅
3. [카드 딜링] 딜러와 플레이어들에게 카드를 2장씩 나누어 준다 ✅
    - 카드는 끗수와 슈트의 조합이다 ✅
    - 각 카드의 개수는 제한이 없다 ✅
    - Blackjack이면 액션이 진행되지 않는다 ✅
4. [플레이어 액션] 각 플레이어들이 점수를 맞추기 위해 카드를 더 받거나 그만 받는다 ✅
    - 점수가 21점을 넘지 않을 경우 얼마든지 카드를 계속 뽑을 수 있다 ✅
    - 카드를 그만 받겠다는 의사 표현 이후 카드를 더 받을 수 없다 ✅
5. [딜러 액션] 딜러는 점수가 17점 이상이 될 때까지 카드를 받는다 ✅
6. [점수 계산] 플레이어들의 점수를 계산한다 ✅
    - A는 1 또는 11로 계산하며, J,Q,K는 10으로 계산한다 ✅
    - 점수가 21점을 넘으면 버스트다 ✅
    - 점수가 21점이면 블랙잭이다 ✅
7. [승패 계산] 딜러와 플레이어의 점수를 비교하여 각 플레이어의 승패를 가린다 ✅
...
8. [플레이어 수익 계산] 승패에 따라 각 플레이어의 수익을 계산한다 ✅
    - 승리한 플레이어는 베팅 금액의 1배를 추가로 받는다 ✅
    - 패배한 플레이어는 베팅 금액을 잃는다 ✅
    - 무승부인 플레이어는 베팅 금액을 돌려받는다 ✅
    - 블랙잭 승리한 플레이어는 베팅 금액의 1.5배를 추가로 받는다 ✅
9. [딜러 수익 계산] 플레이어들의 수익의 합을 이용해 딜러의 수익을 계산한다 ✅
...
- ace 취급
    - (전제) 버스트된다면 ace를 11로 계산했던 걸 1로 취급해서 10을 빼자 -> 버스트된다면 ace는 1로 계산됨
    - (반전) 버스트되지 않는 선에서 ace를 1로 계산했던 걸 11로 취급해서 10을 더하자 -> 버스트되지 않는 선에서 ace는 11로 계산됨

## 메모

### 역할 분담에 대한 고민

- Game : 게임의 각 단계 관리하기(게임참가->카드딜링(deal)->액션(draw, stand)->결과)
- Player/Dealer: 카드 뽑기(deal, draw), 본인의 승패 결정하기
- Cards : 카드 보관하기, 카드 합 계산하기(bust, blackjack)
- Betting : 베팅 금액 보관하기, 수익 계산하기
