...
- [x] 각 플레이어의 최종 수익을 출력한다.
  - [x] 플레이어 승 → 베팅 금액의 2배만큼 수익
  - [x] 플레이어 패 → 베팅 금액 손실
  - [x] 딜러 승 → 플레이어의 베팅 금액 가져감
...
- 사이클1 때보다 수정 범위가 줄었는가/늘었는가
  - 1개 늘었음.
- 규칙 적용으로 변경한 코드 1곳
  - Money라는 객체를 생성하지 않고 Controller에서 바로 생성해 사용하려고 했으나, Player가 Money를 갖는 것이 옳다고 판단해 코드를 변경함.
- 테스트가 설계를 도운 순간 1회
  - 베팅 기능 구현 시 분기 처리되어 베팅 결과가 판정되는 경우가 많은데, 해당 케이스들을 테스트로 먼저 작성해보니 복잡했던 머릿속을 정리하면서 구현할 수 있었음.
