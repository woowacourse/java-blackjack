...
class PlayerTest {
    ...
    void test_receive_card() {
        ...
        AbstractParticipant player = new Player("pobi", state);
    ...
    void test_state_hit_player() {
        State state = new Hit(defaultInitialCards);
        AbstractParticipant player = new Player("suri", state);
    ...
    void calculateWinPrize_player_blackjack_dealer_not() {
        ...
        int money = 10000;
        AbstractParticipant dealer = new Dealer(hitState);
        Player player = new Player("suri", blackJackState);
        player.betting(money);
        assertThat(player.calculateWinPrize(dealer.getState())).isEqualTo((int) (money * 1.5));
    }
    ...
    void calculateWinPrize_player_dealer_blackjack() {
        ...
        assertThat(player.calculateWinPrize(dealer.getState())).isEqualTo(0);
    }
    ...
    void calculateWinPrize_player_not_dealer_blackjack() {
        ...
        assertThat(player.calculateWinPrize(dealer.getState())).isEqualTo(money * -1);
    }
    ...
    void calculateWinPrize_player_dealer_not_blackjack() {
        ...
        player.stay();
        assertThat(player.calculateWinPrize(dealer.getState())).isEqualTo(money);
    }
}
