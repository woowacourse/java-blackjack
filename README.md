...
### 최종 수익
- [ ] 플레이어가 Blackjack인 경우
  - [ ] 딜러가 Blackjack이면 수익은 0이다.
  - [ ] 딜러가 Stay 혹은 Bust이면 수익은 (배팅 금액 x 1.5배)이다.
- [ ] 플레이어가 Stay인 경우
  - [ ] 딜러가 BlackJack이면 수익은 (배팅 금액 x -1)이다.
  - [ ] 딜러가 Bust이면 수익은 (배팅 금액 x 1)이다.
  - [ ] 딜러가 Stay인 경우
    - [ ] 딜러 카드의 합이 크면 수익은 (배팅 금액 x -1)이다.
    - [ ] 딜러 카드의 합이 작으면 수익은 (배팅 금액 x 1)이다.
    - [ ] 딜러 카드의 합이 플레이어 카드의 합과 같으면 수익은 0이다.
- [ ] 플레이어가 Bust인 경우
  - [ ] 딜러가 Bust이면 수익은 0이다.
  - [ ] 딜러가 Stay 혹은 BlackJack이면 수익은 (배팅 금액 x -1)이다.
- [ ] [Error] Hit 상태에서 수익을 계산하려하면 예외를 발생시킨다.
