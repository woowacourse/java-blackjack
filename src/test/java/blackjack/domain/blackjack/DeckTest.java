...
class DeckTest {
    ...
    void create_one_pack_with_52_card() {

        assertThatCode(() -> {
            final var sut = Deck.createPack();
            final var size = IntStream.range(0, 52)
                                      .mapToObj(i -> sut.draw())
                                      .toList()
                                      .size();
    ...
    void throw_exception_when_empty() {
        ...
        assertThatThrownBy(sut::draw).isInstanceOf(IllegalStateException.class);
    }
}
