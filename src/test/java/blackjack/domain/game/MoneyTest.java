...
class MoneyTest {
	...
	void multiply_Money(String money, String multiplier, int expectedValue) {
		Money initMoney = new Money(money);
		Money multipliedMoney = initMoney.multiply(new BigDecimal(multiplier));
		assertThat(multipliedMoney.getValue().intValue()).isEqualTo(expectedValue);
	}
	...
	void get_Minus_Value() {
		...
		assertThat(money.getMinusValue().getValue()).isEqualTo(new BigDecimal("-1000"));
	}
}
