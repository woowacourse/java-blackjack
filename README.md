...
- [x] 주어진 승무패 결과를 전달받아서 상대방의 결과를 반환한다.

* 플레이어 판정 결과 목록

- [x] 각 플레이어의 이름에 따른 승/무/패 결과를 저장한다.
- [x] 딜러의 승/무/패 결과를 합산해서 반환한다.
...
- [ ] 클래스 책임 검토, 필요에 따라 분리
    - [x] Participants 클래스 책임 분리
        ...
        - [x] BlackJackGame에서 참가자를 꺼내 로직을 수행하게 할지 / Participants 내부에서 처리할지 결정
            - 후자로 구현했으나 BlackJackGame이 사실상 매개만 하는 계층이 되고, Participants의 책임이 너무 커져서 전자로 수정함.
    - [x] Cards 일급콜렉션 객체 분리
        - [x] Cards 객체에서 카드의 문자 합 계산하도록 책임 이관
        - [ ] Score 값 객체 분리
    - [x] 두 값을 비교해 승무패를 결정하는 클래스 분리
    - [ ] 게임 상태를 조회하는 도메인 로직 단순화
