...
class PlayerResultTest {
	...
	class PlayerWin {
		...
		void blackJackProfitTest() {
			Player blackJackPlayer = TestData.getBlackJackPlayer(100);
			...
			assertThat(result.getProfit())
				.isEqualTo((int)(blackJackPlayer.getBet() * ResultState.BLACKJACK.getMultiplier()));
		}

		@Test
		@DisplayName("높은 점수로 승리한 플레이어는 건 돈 만큼의 수익을 올려야 한다.")
		void playerWinProfitTest() {
			Player score20Player = TestData.getScore20Player(100);
	...
	class PlayerDraw {
		...
		public void playerDealerDrawProfitTest() {
			...
			assertThat(result.getProfit())
				.isEqualTo((int)(score20Player.getBet() * ResultState.DRAW.getMultiplier()));
		}
		...
		public void bothBlackJackProfitTest() {
			...
			assertThat(result.getProfit())
				.isEqualTo((int)(blackJackPlayer.getBet() * ResultState.DRAW.getMultiplier()));
		}
