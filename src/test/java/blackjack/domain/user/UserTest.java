...
public class UserTest {
    ...
    void validate_range() {
        User user = mock(User.class, withSettings().useConstructor(new UserName("aki")).defaultAnswer(CALLS_REAL_METHODS));
        user.hit(Card.of(CardNumber.EIGHT, CardType.CLOVER));
        user.hit(Card.of(CardNumber.FIVE, CardType.SPADE));
        user.hit(Card.of(CardNumber.TEN, CardType.SPADE));
    ...
    void hit_test() {
        ...
        assertThat(hand.get(0)).isEqualTo(Card.of(CardNumber.EIGHT, CardType.CLOVER));
        assertThat(hand.get(1)).isEqualTo(Card.of(CardNumber.FIVE, CardType.SPADE));
        assertThat(hand.get(2)).isEqualTo(Card.of(CardNumber.TEN, CardType.SPADE));
    }

    @Test
    @DisplayName("User 클래스가 가진 카드의 점수를 계산하여 반환한다.")
    void get_score() {
        User user = mock(User.class, withSettings().useConstructor(new UserName("aki")).defaultAnswer(CALLS_REAL_METHODS));
        user.hit(Card.of(CardNumber.EIGHT, CardType.CLOVER));
        user.hit(Card.of(CardNumber.THREE, CardType.SPADE));
    ...
    void ace_calculate_11() {
        User user = mock(User.class, withSettings().useConstructor(new UserName("aki")).defaultAnswer(CALLS_REAL_METHODS));
        user.hit(Card.of(CardNumber.TEN, CardType.CLOVER));
        user.hit(Card.of(CardNumber.ACE, CardType.SPADE));
    ...
    void ace_calculate_1() {
        User user = mock(User.class, withSettings().useConstructor(new UserName("aki")).defaultAnswer(CALLS_REAL_METHODS));
        user.hit(Card.of(CardNumber.TEN, CardType.CLOVER));
        user.hit(Card.of(CardNumber.THREE, CardType.HEART));
        user.hit(Card.of(CardNumber.ACE, CardType.SPADE));
