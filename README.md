...
- [x] 플레이어는 게임을 시작할 때 배팅 금액을 정해야 한다.
  - [x] 베팅 금액 제약
    - [x] 최소 베팅 금액은 10원이며, 10원 단위로만 베팅 가능하다.
    - [x] 최대 베팅 금액은 10억 원($10^9$)으로 제한한다.
    - [x] 금액이 `null`이거나 숫자가 아닌 경우 예외가 발생한다.
  - [x] 수익
    - [x] 수익 상한선: 시스템 안정성을 위해 개별 수익금의 절대값은 100조 원($10^{14}$)을 초과할 수 없다.
  - [x] 베팅판
    - [x] 플레이어별 베팅 금액을 가진다.
    - [x] 수익률을 사용해서 이익 계산이 가능하다.
- [x] 승패 결과에 따른 수익 계산 기능을 제공한다.
  - [x] 수익률 계산 규칙
    - [x] 블랙잭 승리: 베팅 금액의 1.5배 수익 (수익률 1.5)
    - [x] 일반 승리: 베팅 금액의 1배 수익 (수익률 1.0)
    - [x] 무승부: 베팅 금액 보존 (수익률 0)
    - [x] 패배: 베팅 금액 전액 손실 (수익률 -1.0)
  - [x] 최종 정산
    - [x] 딜러의 총 수익은 모든 플레이어 수익 합계의 반전값(`sum * -1`)이다.
