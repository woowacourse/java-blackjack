...
- [x] 딜러와 각 플레이어 간 승패 결정
    - [] 블랙잭일 경우 승리
    - [] 블랙잭이 아니라면 21에 가까운 쪽이 승리한다.
    - [] 둘 다 블랙잭이라면 무승부로 처리한다.
    - [x] 같은 값일 경우 무승부
    - [x] 각 참가자의 최종 승패 결과 출력
- [] 게임에 승리 결과에 따라 수익을 처리
  - [] 플레이어가 첫 두 장의 카드가 블랙잭이라면 베팅금액의 1.5배를 받는다.
  - [] 단, 딜러와 플레이어 모두 블랙잭이라면 플레이어는 베팅 금액을 돌려 받는다.
  - [] 딜러가 버스트된다면, 플레이어 승패와 상관없이 베팅 금액을 받는다.
  - [] 승자는 배팅 금액을 가져간다.
  - [] 패자는 배팅 금액을 잃는다.
  - [] 승패가 결정되지 않는다면, 돈을 돌려 받는다.
