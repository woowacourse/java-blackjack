...
- 배팅 등록
    ...
    - [x] 배팅 금액은 최소 0원을 초과해야한다, 이외 범위는 `IllegalArgumentException`을 발생시킨다.

- 게임 초기화
    - [x] 게임 시작 시, 모든 카드를 덱에 등록하고 셔플한다.
    - [x] 딜러와 플레이어에게 카드를 각각 2장씩 배분한다.
        - [x] 초기 2장의 카드 합이 21인 경우 블랙잭(Blackjack)으로 판정한다.
...
- 수익 계산
    - [x] 플레이어의 수익률 결정:
        ...
        - 패배: 배팅 금액 전액 손실 (-1.0배)
    - [x] 딜러의 최종 수익 계산: 모든 플레이어의 수익 합산 결과의 역산
